...
if (isset($_POST['username'])) {
    ...
    $db = new Database();
    $query = $db->connect()->prepare('DELETE FROM users WHERE username = :username');

    if ($query->execute(['username' => $username])) {
        PRINT <<<HERE
        <div class="alert alert-danger" role="alert">¡Se ha eliminado con éxito!</div>
        HERE;
    }else{
        PRINT <<<HERE
        <div class="alert alert-success" role="alert">¡Error, algo salió mal!</div>
        HERE;
    }
...
<body>
    ...
    <header>
        <div class="container-fluid bg-light" id="intro">
            <br>
            <br>
            <h1 class="text-center">Eliminar a usuario de<br><img src="./img/LogoFaze.svg" height="50px"></h1>
            <br>
            <br>
        </div>
    </header>
